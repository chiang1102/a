...
class Spider(Spider):
    def __init__(self):
        super().__init__()
        self.base_url = 'https://api-h5.uvod.tv'
        self.web_url = 'https://www.uvod.tv'
        self.token = ''
        self._iv = b"abcdefghijklmnop"
...
"""
    ...
    def _build_headers(self, path: str, payload: dict):
        ...
        if path == '/video/latest':
            parent_id = payload.get('parent_category_id', 101)
            ...
            if 'keyword' in filtered_params:
                filtered_params['keyword'] = quote(str(filtered_params['keyword']), safe='').lower()
            query = urlencode(sorted(filtered_params.items()), doseq=True).lower()
            text = f"{token}-{query}-{ts}"

        elif path == '/video/info': 
            text = f"-id={payload.get('id', '')}-{ts}"
        elif path == '/video/source': 
            quality = payload.get('quality', '')
            fragment_id = payload.get('video_fragment_id', '')
            video_id = payload.get('video_id', '')
            text = f"-quality={quality}&video_fragment_id={fragment_id}&video_id={video_id}-{ts}"
        else: 
            filtered = {k: v for k, v in (payload or {}).items() if v not in (0, '0', '', False, None)}
            query = urlencode(sorted(filtered.items()), doseq=True).lower()
            text = f"{token}-{query}-{ts}"
    ...
    def homeContent(self, filter):
        ...
        lst = (data.get('list') or data.get('category') or []) if isinstance(data, dict) else (data or [])
        classes = []
        for it in lst:
        ...
        if not classes: 
            classes = [
                ...
                {'type_name': '体育', 'type_id': '104'}, {'type_name': '纪录片', 'type_id': '105'}, 
                {'type_name': '粤台专区', 'type_id': '106'},{'type_name': '兒童', 'type_id': '107'},
                {'type_name': '午夜版', 'type_id': '108'}
            ]
        ...
        f_lang = {
            ...
            "name": "語言",
            "value": [
                {"n": "全部", "v": ""}, {"n": "國語", "v": "国语"}, {"n": "粵語", "v": "粤语"},
                {"n": "英語", "v": "英语"}, {"n": "韓語", "v": "韩语"}, {"n": "日語", "v": "日语"},
                {"n": "閩南語", "v": "闽南语"}, {"n": "泰語", "v": "泰语"}, {"n": "其它", "v": "其它"}
            ]
        }
        year_values = [{"n": "全部", "v": ""}]
        for y in range(2025, 2004, -1):
            year_values.append({"n": str(y), "v": str(y)})
        year_values.append({"n": "更早", "v": "2004-1900"})
        f_year = {"key": "year", "name": "年份", "value": year_values}

        f_state = {
            "key": "state",
            "name": "狀態",
            "value": [{"n": "全部", "v": ""}, {"n": "連載中", "v": "0"}, {"n": "已完結", "v": "1"}]
        }
        
        f_sort = {
            "key": "sort_type",
            "name": "排序",
            "value": [{"n": "最新", "v": "asc"}, {"n": "最熱", "v": ""}]
        }

        filter_list = [f_region, f_lang, f_year, f_state, f_sort]
        for item in classes:
            filters[item['type_id']] = filter_list
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        if isinstance(extend, dict):
            for k in ['category_id', 'year', 'region', 'state', 'keyword', 'language', 'sort_type']:
                if extend.get(k): 
                    val = extend[k]
        ...
        return {'list': videos, 'page': page, 'pagecount': 9999, 'limit': 42, 'total': total}
